...
<div class="card"><div class="card-body table-responsive p-0"><table class="table table-striped"><thead><tr><th>Date</th><th>Total Hours</th><th>Total Diesel</th><th>Total Earning</th><th>Diesel Cost</th><th>Net</th></tr></thead><tbody>@foreach($rows as $row)<tr><td>{{ \Carbon\Carbon::parse($row['date'])->format('d-M-y') }}</td><td>{{ number_format($row['total_hours'], 2) }}</td><td>{{ number_format($row['total_diesel'], 2) }}</td><td>{{ number_format($row['total_earning'], 2) }}</td><td>{{ number_format($row['diesel_cost'], 2) }}</td><td>{{ number_format($row['net'], 2) }}</td></tr>@endforeach</tbody></table></div></div>
@stop
